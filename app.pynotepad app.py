...
def extract_pdf_info(pdf_path):
    try:
        ...
        message = ai_client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""以下の依頼書テキストから情報を抽出してください。
必ずJSON形式のみで返答してください。

{{
  "request_date": "依頼日（YYYY/MM/DD形式）",
  "work_content": "作業内容（作業区分と作業内容を簡潔に）",
  "amount": "合計金額（数字のみ、例：7000）",
  "tantou": "ご担当者名",
  "user_name": "ユーザー名（作業対象のお客様名）",
  "work_place": "作業場所（住所）"
}}
...
def fetch_mail_pdfs(imap_server, address, password, days=90):
    ...
    try:
        ...
        for mail_id in mail_ids:
            ...
            domain = from_addr.split('@')[-1].replace('>', '').strip() if '@' in from_addr else ''
...
def fetch_mail():
    ...
    flash(f'メール取込完了：{count}件登録', 'success')
...
def upload():
    ...
    db.session.add(order)
    db.session.commit()
    flash(f'管理番号 {management_no} を登録しました。AI読み取り完了！', 'success')
    return redirect(url_for('index'))
...
def edit_order(order_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash(f'管理番号 {order.management_no} を更新しました', 'success')
        return redirect(url_for('index'))
...
def export_client(client_id):
    ...
    for o in orders:
        ws.append([
            o.management_no, o.request_date, o.user_name, o.work_place,
            o.work_content, o.tantou,
            o.completed_at.strftime('%Y/%m/%d') if o.completed_at else '',
            o.amount
        ])
        total += o.amount or 0
    ws.append(['', '', '', '', '', '', '合計', total])
    path = f'invoice_{client.name}.xlsx'
    wb.save(path)
...
def export_all():
    ...
    for o in orders:
        ws.append([
            o.management_no, o.client.name, o.request_date, o.user_name,
            o.work_place, o.work_content, o.tantou, o.amount or 0,
